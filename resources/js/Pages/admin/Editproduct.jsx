...
const Editproduct = ({ singleData }) => {
    ...
    useEffect(() => {
        if (singleData) {
            setitem({
                ...
                'featur': singleData.featur,
                'discription': singleData.discription,
                'price': singleData.price,
                'cover': singleData.packing,
                'product': singleData.Product,
            });
        }
    }, [singleData]);

    let storComingImg = "/storage/" + item.image;
    useEffect(() => {
        if (item.image.length > 0) {
            setshowImg(storComingImg)
        }
    })
    ...
    const HandleuplaodData = (evt) => {
        evt.preventDefault()
        if (!item.title || !item.type || !item.unit || !item.country || !item.featur || !item.price || !item.cover || !item.product) {
            toast.error('Please fill in all the fields');
            return;
        }
        router.post(`/updateproduct/${singleData.id}`, item)
    }
